...
class SneakerBot(object):
    ...
    def run(self):
        ...
        if not self.CONFIG.bypass_stock_check:
            ...
            print "bypass_stock_check set to False. Bypassing stock check may throw an error if size and quantity " \
                  "not available or if quantity is set to more than the maximum order limit." \
